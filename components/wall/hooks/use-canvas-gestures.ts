...
export function useCanvasGestures({
  ...
  const isElementTarget = useCallback(
  ...
  const handleCanvasPointerDown = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      const target: PointerTarget = isElementTarget(event.target)
        ? "element"
        : "canvas";
  ...
  const handleCanvasPointerMove = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (
        primaryPointer.target === "element" ||
        secondaryPointer.target === "element"
      ) {
        return;
      }
  ...
  const wasDragging = useCallback(
    () => Date.now() - lastDragEndRef.current < RECENT_DRAG_WINDOW_MS,
    []
  );
