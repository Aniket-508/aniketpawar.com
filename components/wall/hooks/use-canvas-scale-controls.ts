...
const getCanvasElement = (canvasRef: RefObject<HTMLDivElement | null>) => {
...
export const useCanvasScaleControls = ({
  ...
  const setScale = useCallback((value: SetStateAction<number>) => {
  ...
  const clampScale = useCallback(
    (value: number) => clamp(value, MIN_SCALE, MAX_SCALE),
    []
  );
  ...
  const scaleByAtPoint = useCallback(
    (clientX: number, clientY: number, factor: number) => {
      const previousScale = scaleRef.current;
      const nextScale = Number.parseFloat((previousScale * factor).toFixed(3));
  ...
  const zoomIn = useCallback(() => {
    ...
    scaleByAtPoint(x, y, 1 + ZOOM_STEP);
  }, [getViewportCenter, scaleByAtPoint]);

  const zoomOut = useCallback(() => {
    const { x, y } = getViewportCenter();
    scaleByAtPoint(x, y, 1 / (1 + ZOOM_STEP));
  }, [getViewportCenter, scaleByAtPoint]);
