export const decimateTimeSeries = <T extends Record<string, unknown>>(
  data: T[],
  maxPoints: number,
  valueKeys: string[] = []
): T[] => {
...
export const maxRenderPointsForWidth = (innerWidth: number): number =>
  Math.max(64, Math.ceil(innerWidth * 1.5));

/** Bucket OHLC rows into fewer candles while preserving high/low extremes. */
export const decimateOhlcData = <T extends Record<string, unknown>>(
  data: T[],
  maxPoints: number
): T[] => {
  const len = data.length;
