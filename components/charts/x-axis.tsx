...
const gapsToIndices = (gaps: number[]): number[] => {
  const indices = [0];
  let position = 0;
...
const indicesForTickCount = (length: number, tickCount: number): number[] => {
  ...
  const indices = [...new Set(rawIndices)].toSorted((a, b) => a - b);
  if (indices[0] !== 0) {
    indices.unshift(0);
  }
  ...
  return [...new Set(indices)].toSorted((a, b) => a - b);
};
...
const indexGaps = (indices: number[]): number[] => {
  const gaps: number[] = [];
...
const smallestGapEdgePreference = (indices: number[]): number => {
 ...
 */
export const selectEvenlySpacedIndices = (
  length: number,
  targetCount: number,
  options?: {
    data?: Record<string, unknown>[];
...
const XAxisInner = memo(
  ({
    ...
    const labelsToShow = useMemo(() => {
      ...
      if (tickMode === "data" || (xDomain !== null && xDomain !== undefined)) {
    ...
    return createPortal(
      <div className="pointer-events-none absolute inset-0">
        {labelsToShow.map((item) => (
          <XAxisLabel
            animatePosition={xDomain === null || xDomain === undefined}
