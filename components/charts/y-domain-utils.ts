...
export const niceYDomain = (domain: YDomain): YDomain => {
 ...
 */
export const shouldTweenYDomain = (from: YDomain, to: YDomain): boolean => {
  const span = Math.max(
    Math.abs(to[1] - to[0]),
...
export const isLoadingChromePhase = (phase: ChartPhase): boolean =>
  phase === "loading" || phase === "revealingLoading";

/** Phases where grid lines use loading stroke styling (muted / dashed chrome). */
export const isLoadingGridChromePhase = (phase: ChartPhase): boolean =>
  phase === "loading" || phase === "exiting" || phase === "gridTweenLoading";

/** Phases where Y-domain tween runs after the series has exited. */
export const isYDomainTweenPhase = (phase: ChartPhase): boolean =>
  phase === "gridTweenLoading" || phase === "gridTweenReady";

export const resolveAnimatedYDestinationDomains = (
...
export const computeYDomainsByAxis = ({
  lines,
  resolveDomain,
}: {
  lines: LineConfig[];
...
export const mergeYDomainRecords = (
